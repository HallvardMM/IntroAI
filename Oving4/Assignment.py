...
class CSP:
    def __init__(self):
        ...
        # self.backtracks counts amount of backtracks
        self.backtracks = 0

        # self.failedBaktracks counts failed backtracks
        self.failedBacktracks = 0
        ...
        while queue:
            ...
            if self.revise(assignment, xi, xj):
                if not assignment[xi]:
                    return False
                for tup in self.get_all_neighboring_arcs(xi):
                    if tup[0] != xj:
                        queue.append((tup[0], xi))
        return True
    ...
    def revise(self, assignment, i, j):
        ...
        for x in assignment[i]:
            ...
            if not sum(pair in self.constraints[i][j] for pair in pairs):
...
if __name__ == "__main__":
    board=input("What board do you want to play? ")
    if board == "all":
        for name in ["easy", "medium", "hard", "veryhard"]:
            print("Difficulty: ", name)
            csp=create_sudoku_csp(str(name+".txt"))
            print_sudoku_solution(csp.backtracking_search())
            print("Backtracks: ", csp.backtracks)
            print("Failed Backtracks: ", csp.failedBacktracks)
    else:
        print("Difficulty: ", board)
        csp=create_sudoku_csp(str(board+".txt"))
        print_sudoku_solution(csp.backtracking_search())
