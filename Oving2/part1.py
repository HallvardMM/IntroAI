"""File for part 1 with only open cells or blocked cells"""
...
def astar(map,heuristics):
    ...
    while openNodes:
        ...
        for i in range(-1,2):
            for j in range(-1,2):
                if (i!=0 or j!=0) and (i==0 or j==0) and map.get_cell_value([lowestCostNode.y+i,lowestCostNode.x+j])!=-1:
                    distance = lowestCostNode.g+1 # Can add 1 since all tiles have cost 1 or -1.
...
map1.show_map() # Prints map task 1 before A*
endNode = astar(map1, combinedHeuristic)
backTrack(endNode, map1) # Prints map after A*

map2.show_map() # Prints map task 2 before A*
endNode2 = astar(map2, combinedHeuristic)
backTrack(endNode2, map2) # Prints map task 2 after A*
